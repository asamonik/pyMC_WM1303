...
class PathHelper:
    ...
    async def process_path_packet(self, packet):
        ...
        try:
            ...
            mac_and_data = packet.payload[2:]
            client = None
            decrypted = None
            # A routing byte only selects candidates, not a full identity.
            # Like MeshCore and the companion response handler, try every
            # matching peer's secret until one MAC verifies. No client state
            # changes until that authenticated candidate's path is validated.
            for client_info in identity_acl.get_all_clients():
                ...
                if not shared_secret:
                    continue
                try:
                    candidate_data = CryptoUtils.mac_then_decrypt(
                        shared_secret[:16], shared_secret, mac_and_data,
                    )
                except ValueError:
                    # The current core raises ValueError on a mismatched MAC;
                    # a colliding candidate must not prevent trying the next.
                    continue
                if candidate_data:
                    client = client_info
                    decrypted = candidate_data
                    break

            if client is None:
                logger.debug(f"No authenticated PATH client for source 0x{src_hash:02X}, allowing forward")
                return False
