...
# Keep the existing Python writer/admin wire values. A blank-password reader
# needs a distinct role; do not migrate those roles to firmware's 2/3 here.
PERM_ACL_GUEST = 0x00
PERM_ACL_ADMIN = 0x02
...
class ClientInfo:
    ...
    def is_guest(self) -> bool:
        # Existing telemetry callers classify password writers as guests too.
        # Preserve that restriction; posting uses exact writer/admin roles.
        return (self.permissions & PERM_ACL_ROLE_MASK) in (
            PERM_ACL_GUEST, PERM_ACL_READ_WRITE,
        )
...
class ACL:
    ...
    def authenticate_client(
        ...
        if is_room_server:
            ...
            # Missing passwords disable those roles, not read-only admission.
            # The blank-password branch below still enforces allow_read_only.
        else:
        ...
        if not password:
            ...
            if client is None:
                ...
                role = client.permissions & PERM_ACL_ROLE_MASK
                if role not in (PERM_ACL_GUEST, PERM_ACL_READ_WRITE, PERM_ACL_ADMIN):
                    logger.info("Blank password, sender has an unsupported ACL role")
                    return False, 0
                if role == PERM_ACL_GUEST and not self.allow_read_only:
                    logger.info("Blank password, read-only access is disabled")
                    return False, 0
                # Known writers/admins keep ACL-based reconnect access. Blank
                # passwords never create or promote either privileged role.
