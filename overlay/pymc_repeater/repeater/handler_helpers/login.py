...
class LoginHelper:
    ...
    def register_identity(
        ...
        if identity_type == "room_server":
            ...
            validate_room_settings(settings, path=f"Room {name!r} settings")
            ...
            # A room may intentionally be passwordless/read-only. Register it
            # even when that admission is disabled; ACL authentication, not
            # handler absence, decides whether a client may log in.
            if not admin_password and not guest_password:
                logger.info(
                    "Room server %r has no passwords; blank read-only access is %s",
                    name, "enabled" if settings.get("allow_read_only", True) else "disabled",
                )
        ...
        def auth_callback_with_context(
            ...
            # Authentication updates the ACL in place. A room session is not
            # usable until its durable sync/reset state is committed too.
            pubkey = client_identity.get_public_key()
            previous_client = identity_acl.get_client(pubkey)
            previous_state = vars(previous_client).copy() if previous_client is not None else None
            ...
            if success and identity_type == "room_server" and self.sqlite_handler is not None:
                admitted_client = identity_acl.get_client(pubkey)
                try:
                    sync_kwargs = {}
                    if sync_since is not None:
                        sync_kwargs["sync_since"] = sync_since
                    saved = self.sqlite_handler.upsert_client_sync(
                        room_hash=f"0x{hash_byte:02X}",
                        client_pubkey=pubkey.hex(),
                        pending_ack_crc=0,
                    ...
                    if saved is not True:
                        raise RuntimeError("Room login sync state was not saved")
                except Exception as e:
                    # No await separates the ACL update, SQL reset and rollback.
                    # Never remove/restore a different client's replacement.
                    if identity_acl.get_client(pubkey) is admitted_client:
                        if previous_client is None:
                            identity_acl.remove_client(pubkey)
                        elif admitted_client is previous_client:
                            admitted_client.__dict__.clear()
                            admitted_client.__dict__.update(previous_state)
                    logger.warning(
                        "Room login rejected: sync state could not be saved for hash=0x%02X (%s)",
                        hash_byte, type(e).__name__,
                    )
                    return False, 0
