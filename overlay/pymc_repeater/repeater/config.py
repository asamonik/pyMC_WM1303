    ...
    if radio_type in ("sx1262", "sx1262_ch341"):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install the pyMC_WM1303 overlay (or run install.sh from "
        ...
        if hasattr(radio, "begin"):
            try:
                ...
                raise RuntimeError(f"Failed to initialize WM1303 radio: {e}") from e

        return radio

    raise RuntimeError(
        f"Unknown radio type: {radio_type}. "
        "Supported: sx1262, sx1262_ch341, kiss (or kiss-modem), pymc_tcp, pymc_usb, wm1303"
    )
