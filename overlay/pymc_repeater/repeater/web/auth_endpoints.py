        ...
        try:
            ...
            if not result.get("saved"):
                ...
                return json.dumps({
                    ...
                    "error": result.get("error", "Failed to save password to config file"),
                }).encode("utf-8")
            live_updated = bool(result.get("live_updated"))
            message = "Password saved. Please log in again with your new password."
            if not live_updated:
                message += " Restart the service to finish applying MeshCore login settings."
                logger.warning("Password saved but live repeater settings did not fully reload")
            logger.info("Admin password changed by user %s", user['username'])
            return json.dumps({
                "success": True, "saved": True, "live_updated": live_updated,
