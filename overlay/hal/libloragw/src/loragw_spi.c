...
#define LGW_BURST_CHUNK     16384

/* Global SPI speed override for capture reads (0 = use default) */
uint32_t lgw_spi_speed_override = 0;
...
int lgw_spi_rb(void *com_target, uint8_t spi_mux_target, uint16_t address, uint8_t *data, uint16_t size) {
    ...
    for (i=0; size_to_do > 0; ++i) {
        ...
        k[1].len = chunk_size;
        /* Apply speed override for CAPTURE_RAM bulk reads */
        if (lgw_spi_speed_override > 0) {
            k[0].speed_hz = lgw_spi_speed_override;
            k[1].speed_hz = lgw_spi_speed_override;
        }
        byte_transfered += (ioctl(spi_device, SPI_IOC_MESSAGE(2), &k) - k[0].len );
