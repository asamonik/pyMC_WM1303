...
int sx1302_pa_lna_lut_configure(struct lgw_conf_board_s * context_board) {
    ...
    if (context_board->full_duplex == true) {
        ...
        err |= lgw_reg_w(SX1302_REG_AGC_MCU_LUT_TABLE_A_LNA_LUT, 0x03);    /* LNA: RADIO_CTRL[1] HIGH in IDLE+RX (prevent FEM shutdown during TX->RX transition) */
    } else {
    ...
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_LUT_TABLE_B_LNA_LUT, 0x03);        /* LNA: RADIO_CTRL[7] HIGH in IDLE+RX (prevent FEM shutdown during TX->RX transition) */
...
int sx1302_lora_modem_configure(uint32_t radio_freq_hz) {
    ...
    /* Set preamble size to 8 (to handle 12 for SF5/SF6 and 8 for SF7->SF12) */
    err |= lgw_reg_w(SX1302_REG_RX_TOP_TXRX_CFG7_PREAMBLE_SYMB_NB,  0); /* MSB */
    err |= lgw_reg_w(SX1302_REG_RX_TOP_TXRX_CFG6_PREAMBLE_SYMB_NB, 8); /* LSB */
...
/* -----------------------------------------------------------------------
 * sx1302_agc_reload - Fast AGC MCU restart after TX (~50ms recovery)
 * Instead of waiting 60-90s for natural AGC self-recovery, this function
 * restarts the AGC MCU firmware in-place (FW stays in SRAM) and
 * re-initialises gain parameters, restoring RX sensitivity quickly.
 * ----------------------------------------------------------------------- */
int sx1302_agc_reload(lgw_radio_type_t radio_type) {
    int err = LGW_REG_SUCCESS;
    ...
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_CTRL_FORCE_HOST_FE_CTRL, 1);
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_RF_EN_A_PA_EN, 0);
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_RF_EN_A_LNA_EN, 1); /* LNA ON */

    /* Step 2: stop AGC MCU (firmware stays in SRAM) */
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_CTRL_MCU_CLEAR, 0x01);
    wait_ms(2);
    /* Step 3: restart AGC MCU */
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_CTRL_MCU_CLEAR, 0x00);

    /* Step 4: release FEM control back to restarted AGC */
    err |= lgw_reg_w(SX1302_REG_AGC_MCU_CTRL_FORCE_HOST_FE_CTRL, 0);

    /* Step 5: re-initialise AGC gain parameters via mailbox */
    err = sx1302_agc_start(fw_ver, radio_type,
                           SX1302_AGC_RADIO_GAIN_AUTO, SX1302_AGC_RADIO_GAIN_AUTO,
                           false, false);
    if (err != LGW_REG_SUCCESS)
        printf("WARNING: [agc_reload] agc_start failed\n");
    else
        printf("INFO: [agc_reload] done - LNA active, RX ready\n");
