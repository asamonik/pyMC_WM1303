...
class TextMessageHandler(BaseHandler):
    ...
    def __init__(
        ...
        self._ack_tasks = set()
        self._track_task = track_task_fn or self._track_ack_task

    def _track_ack_task(self, task):
        self._ack_tasks.add(task)

        def finished(done):
            self._ack_tasks.discard(done)
            if not done.cancelled() and done.exception() is not None:
                self.log("Text ACK task failed")
    ...
    async def _send_delayed_ack(self, pkt, delay_s, timestamp_int) -> None:
        ...
        try:
            if await self.send_packet(pkt, wait_for_ack=False) is False:
                self.log("ACK packet transmission was rejected")
                return
            self.log(
    ...
    async def __call__(self, packet: Packet) -> HandlerResult:
        ...
        self.log(f"Received TXT_MSG: type={txt_type}, text_bytes={visible_len}")
        ...
        if txt_type == TXT_TYPE_CLI_DATA:
            ...
            if callback is not None:
                ...
                try:
                    ...
                    self.log(
                        f"Command response captured from {matched_contact.name}: {visible_len} bytes"
                    )
